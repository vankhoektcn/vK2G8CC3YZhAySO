         $(document).ready(function() {
               ...
               setControlFind($.mkv.queryString("idkhoachinh"));
                 $("#luu").click(function () {
                   $(this).Luu({
                         ajax:"../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=Luu"
                      },null,function () {
                           $.LuuTable({
                                 ajax:"../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=luuTableHS_TonKhoViewDetail&IdTonKho=" + $.mkv.queryString("idkhoachinh"),
                                 tablename:"gridTable"
                           });
                      });
                });
                ...
                $("#xoa").click(function () {
                   $(this).Xoa({
                         ajax:"../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=xoa"
                    },null,function () {
                         loadTableAjaxHS_TonKhoViewDetail('');
                         SetDeaultValue();
                     });
               ...
               $("#btnXuatExel").click(function(){
                 ...
                 if( LoaiThuocID==null ||LoaiThuocID=="" ||      mkv_LoaiThuocID==null ||mkv_LoaiThuocID==""
                    ...
                    )
                    {
                      ...
                      $.ajax({
                        type:"POST",
                        url:"../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=XuatExcel"
                 ...
                 if( IdTonKho==null ||IdTonKho==""  
                    )
                    {
                      ...
                      $.ajax({
                        ...
                        cache:false,
                        type:"POST",
                        url:"../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=DieuChinhKho"
                                 +"&IdTonKho="+IdTonKho
                                 ,
                        success:function(data){
                            if(data!=null && data!=""){
            ...
            //───────────────────────────────────────────────────────────────────────────────────────
                $("#help").click(function(){
                    $(this).TimKiem({
                        ajax:"../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=help",title:"Hướng dẫn ",width:"1000px",height:"400px"
                    });    
                });
           ...
           function setControlFind(idkhoatimkiem) {
              if(idkhoatimkiem != "" && idkhoatimkiem != null){
                 $.BindFind({ajax:"../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=setTimKiem&idkhoachinh="+idkhoatimkiem},null,function () {
          ...
          function Find(control,page) {
              if(page == null)page = "1";
              $(control).TimKiem({
                     ajax:"../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=TimKiem&page="+page
               });
          }
         function xoaontable(control,bool){
           if(bool || bool == null)
              $(control).XoaRow({
                 ajax:"../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=xoaHS_TonKhoViewDetail"
              });
         }
         function loadTableAjaxHS_TonKhoViewDetail(idkhoa,page)
         {
             if(idkhoa == null) idkhoa = "";
              ...
              var denphut = $("#denphut").val();
                 ...
                 type:"GET",
                 cache:false,
                 url:"../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=loadTableHS_TonKhoViewDetail&IdTonKho="+idkhoa+"&page="+page
                 +"&TuNgay="+$("#TuNgay").val()
                 +"&DenNgay="+$("#DenNgay").val()
                 +"&IdKho="+$("#IdKho").val()
...
{
 $(obj).unautocomplete().autocomplete("../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=IdNuocSX_NSearch",{
 minChars:0,
 width:150,
 scroll:true,
 header:false,
...
//───────────────────────────────────────────────────────────────────────────────────────
         ...
         {
             $(obj).unautocomplete().autocomplete("../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=IdKhoSearch"
...
//───────────────────────────────────────────────────────────────────────────────────────         
         ...
         {
             $(obj).unautocomplete().autocomplete("../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=LoaiThuocIDSearch",{
             minChars:0,
             width:350,
             scroll:true,
             formatItem:function (data) {
...
//───────────────────────────────────────────────────────────────────────────────────────         
         function IdThuocSearch(obj)
         {
             ...
             $(obj).unautocomplete().autocomplete("../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=IdThuocSearch",{
             minChars:0,
             width:350,
             scroll:true,
             formatItem:function (data) {
         ...
         { 
            ...
            $.BindFind({
                        ajax:"../ajax/HS_TONKHO_Save_ajax3_new.aspx?do=SetDeaultValue"
                                        +"&idkhoa="+$.mkv.queryString("idkhoa")
                                        +"&IdKho="+$.mkv.queryString("IdKho")
...
function checkAllDieuChinh(obj) {
    ...
    for (var i = 0; i < tableCLS.length; i++) {
        if ($(obj).is(":checked")) {
            tableCLS.eq(i).find("td").eq(5).find("input[type=checkbox]").attr("checked", true);
        } else {
            tableCLS.eq(i).find("td").eq(5).find("input[type=checkbox]").attr("checked", false);
        }
    }
}