 $(document).ready(function() {
       ...
       setControlFind($.mkv.queryString("idkhoachinh"));
         $("#luu").click(function () {
           $(this).Luu({
                 ajax:"../ajax/phieuxuatkho_traNCC_ajax3.aspx?do=Luu&idkho="+$.mkv.queryString("idkho")
              },null,function () {
                   $.LuuTable({
        ...
        $("#btnPrint").click(function () {
                ...
                if(idphieuxuat==null ||idphieuxuat=="") return false;
                      $.ajax({
                        type:"POST",
                        url:"../ajax/phieuxuatkho_traNCC_ajax3.aspx?do=PrintPhieuXuatKho"
          ...
          $("#btnPrintBienBan").click(function () {
                var idphieuxuat=$.mkv.queryString("idkhoachinh");
                if(idphieuxuat==null ||idphieuxuat=="") return false;
                      $.ajax({
                        type:"POST",
                        url:"../ajax/phieuxuatkho_traNCC_ajax3.aspx?do=PrintBienBan"
                                 +"&idphieuxuat=" + idphieuxuat,
                        dataType:"text",
                        success:function(data){
                            window.open(data,"_blank");
  ...
  function Find(control,page) {
      if(page == null)page = "1";
      $(control).TimKiem({
             ajax:"../ajax/phieuxuatkho_traNCC_ajax3.aspx?do=TimKiem&page="+page + "&IdKho=" + $.mkv.queryString("IdKho") ,width:"1000",title:"Danh sách phiếu xuất trả nhà cung cấp"
       },null,function(data){
            if(data==null || data==""){
                $.mkv.closeDivTimKiem();
            }
...
function setIdKho()
 {
    ...
    $.BindFind({
        ajax:"../ajax/phieuxuatkho_traNCC_ajax3.aspx?do=setIdKho"
 ...
 function idthuocSearch(obj)
 {
     ...
     header:  "<div style =\"color:#000;position:absolute;top:0px;left:-2px;z-index:1000;background-color:#cfcfcf;border:1px solid black;width:97%;height:30px;padding-right:25px\">"
                    ...
                    + "<div style=\"width:7%;height:30px;color:#000;font-weight:bold;float:left\" >Số HĐ</div>"
                    + "<div style=\"width:20%;height:30px;color:#000;font-weight:bold;float:left\" >Ngày HĐ</div>"
                    + "<div style=\"width:7%;height:30px;color:#000;font-weight:bold;float:left;\" >ĐVT</div>"
                    + "<div style=\"width:8%;height:30px;color:#000;font-weight:bold;float:left\" >SLTồn </div>"
...
{
    $(obj).unautocomplete().autocomplete("../ajax/phieuxuatkho_traNCC_ajax3.aspx?do=IdLoaiThuocSearch" + "&LoaiThuocID="+$.mkv.queryString("LoaiThuocID"),{
    minChars:0,
    width:350,
    scroll:true,
    formatItem:function (data) {
...
function tinhthanhtien(obj)
{
    ...
    var thanhtientruocthue=dongia * sl;
    $("#gridTable").find("tr").eq($(obj).parent().parent().index()).find("#thanhtientruocthue").val(formatCurrency(thanhtientruocthue));
    var thanhtiensauthue=(thanhtientruocthue*eval($("#gridTable").find("tr").eq($(obj).parent().parent().index()).find("#VAT").val().replace(/\./g,'').replace(/\,/g,'.'))/100) + thanhtientruocthue;
