 $(document).ready(function() {
     $('input[id^=luuTable]').click(function () {
        $(this).LuuTable({ajax:"../ajax/DMKhoa_ajax2.aspx?do=luuTable",tablename:"gridTable"});
     });
 ...
 function xoa(control,bool){
    if(bool || bool == null)
      $(control).XoaRow({ajax:'../ajax/DMKhoa_ajax2.aspx?do=xoa'});
 }
  function Find(control) {
      $(control).TimKiem({
             ajax:"../ajax/DMKhoa_ajax2.aspx?do=TimKiem",dataType:'json',showPopup:false
       },function () {
            ...
            html+="<th>STT</th>";
            html+="<th></th>";
            html+="<th>maphongkhambenh</th>";
            html+="<th>tenphongkhambenh</th>";
            html+="<th>kyhieu</th>";
            html+="<th>IsNoiTru</th>";
            html+="<th></th>";
            html+="</table>";
           ...
           result:function(json,item){
                ...
                for( var i = -1, y = item.length; ++i != y;){ 
                      ...
                      html[j++]="<td><input mkv='true' id='maphongkhambenh' type='text' value='" + item[i].MAPHONGKHAMBENH + "' style='width:100px' " + (!edit ? "disabled" : "") + "/></td>";
                      ...
                      html[j++]="<td><input mkv='true' id='IsNoiTru' type='checkbox' " + (item[i].ISNOITRU == "True" ? "checked" : "") + " " + (!edit ? "disabled" : "") + "/></td>";
