 ...
 $(document).ready(function() {
     $('input[id^=luuTable]').click(function () {
        $(this).LuuTable({ajax:"../ajax/banggiadichvu_KTC_ajax.aspx?do=luuTable",tablename:"gridTable"});
     });
 ...
 function xoa(control,bool){
    if(bool || bool == null)
      $(control).XoaRow({ajax:'../ajax/banggiadichvu_KTC_ajax.aspx?do=xoa'});
 }
  function Find(control) {
      $(control).TimKiem({
             ajax:"../ajax/banggiadichvu_KTC_ajax.aspx?do=TimKiem",dataType:'json',showPopup:false
       },function () {
...
//                      html[j++]="<td><input mkv='true' id='tenbaohiem' type='text' value='" + item[i].TENBAOHIEM + "' style='width:400px' " + (!edit ? "disabled" : "") + "/></td>";
 ...
 {
     $(obj).unautocomplete().autocomplete("../ajax/banggiadichvu_KTC_ajax.aspx?do=idphongkhambenhSearch",{
     minChars:0,
     width:350,
     scroll:true,
     addRow:true,
 ...
 {
     $(obj).unautocomplete().autocomplete("../ajax/banggiadichvu_KTC_ajax.aspx?do=IDDVTSearch",{
     minChars:0,
     width:350,
     scroll:true,
     addRow:true,
