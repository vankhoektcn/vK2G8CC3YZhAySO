 $(document).ready(function() {
     $('input[id^=luuTable]').click(function () {
        $(this).LuuTable({ajax:"../ajax/kb_phongkham_ajax2.aspx?do=luuTable",tablename:"gridTable"});
     });
 ...
 function xoa(control,bool){
    if(bool || bool == null)
      $(control).XoaRow({ajax:'../ajax/kb_phongkham_ajax2.aspx?do=xoa'});
 }
  function Find(control) {
      $(control).TimKiem({
             ajax:"../ajax/kb_phongkham_ajax2.aspx?do=TimKiem",dataType:'json',showPopup:false
       },function () {
             $("#tableAjax_kb_phongkham").html('<img src="../images/loading.gif" style="margin:0 41%;padding:10px 0 10px 0"/>'); 
             return true;
       },function (data) {
            var html = "";
            ...
            html+="</table>";
            html+="<div id='paging' style='width:100%;height:50px'/>";
            $("#tableAjax_kb_phongkham").html(html);
      }).myfilter({
           txtfilter:"#gridTable [name=search]",
           paging:function(json,data){
                $("#tableAjax_kb_phongkham").find("#paging").html(data);
           },
           result:function(json,item){
                ...
                for( var i = -1, y = item.length; ++i != y;){ 
                      ...
                      html[j++]="<td><input mkv='true' id='PhongID' type='hidden' value='" + item[i].PHONGID + "'/><input mkv='true' id='mkv_PhongID' type='text' value='"+ item[i].TENPHONG +"' onfocus='PhongIDSearch(this);' class=\"down_select\" style='width:90%' " + (!edit ? "disabled" : "") + "/></td>";
 ...
 {
     $(obj).unautocomplete().autocomplete("../ajax/kb_phongkham_ajax2.aspx?do=PhongIDSearch",{
     minChars:0,
     width:350,
     scroll:true,
     addRow:true,
 ...
 {
     $(obj).unautocomplete().autocomplete("../ajax/kb_phongkham_ajax2.aspx?do=idbacsiSearch",{
     minChars:0,
     width:350,
     scroll:true,
     addRow:true,
 ...
 {
     $(obj).unautocomplete().autocomplete("../ajax/kb_phongkham_ajax2.aspx?do=idphongkhambenhSearch",{
     minChars:0,
     width:350,
     scroll:true,
     addRow:true,
