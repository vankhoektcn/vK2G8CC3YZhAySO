 $(document).ready(function() {
     $('input[id^=luuTable]').click(function () {
        $(this).LuuTable({ajax:"../ajax/KB_LoaiBN_ajax2.aspx?do=luuTable",tablename:"gridTable"});
     });
 ...
 function xoa(control,bool){
    if(bool || bool == null)
      $(control).XoaRow({ajax:'../ajax/KB_LoaiBN_ajax2.aspx?do=xoa'});
 }
  function Find(control) {
      $(control).TimKiem({
             ajax:"../ajax/KB_LoaiBN_ajax2.aspx?do=TimKiem",dataType:'json',showPopup:false
       },function () {
             $("#tableAjax_KB_LoaiBN").html('<img src="../images/loading.gif" style="margin:0 41%;padding:10px 0 10px 0"/>'); 
             return true;
       },function (data) {
            var html = "";
            ...
            $("#tableAjax_KB_LoaiBN").html(html);
      }).myfilter({
           txtfilter:"#gridTable [name=search]",
           paging:function(json,data){
                $("#tableAjax_KB_LoaiBN").find("#paging").html(data);
           },
           result:function(json,item){
                ...
                for( var i = -1, y = item.length; ++i != y;){ 
                      ...
                      html[j++]="<td><input mkv='true' id='TTUT' type='text' value='" + item[i].TTUT + "' onblur='TestSo(this,false,false);' style='width:100%' " + (!edit ? "disabled" : "") + "/></td>";
                      html[j++]="<td><input mkv='true' id='MALOAIBN' type='text' value='" + item[i].MALOAIBN + "' style='width:100%' " + (!edit ? "disabled" : "") + "/></td>";
