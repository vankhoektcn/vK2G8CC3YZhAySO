...
//                 $.BindFind({ajax:"../ajax/zBaoCaoBNRaVienChuyenVien_ajax1.aspx?do=setTimKiem&idkhoachinh="+idkhoatimkiem,dataType:'json'});
//              }      
          }
           function Find(control) {
               $(control).TimKiem({
                     ajax:"../ajax/zBaoCaoBNRaVienChuyenVien_ajax1.aspx?do=TimKiem&idkhoa="+$.mkv.queryString("IdKhoa")+"",dataType:'json',showPopup:false
               },function () {
                    ...
                    return true;
                      ...
                      html+="<th>"+ data.TABLE[0].SONGAYDIEUTRI +"</th>";
                      html+="<th>"+ data.TABLE[0].CHUYENDENBENHVIEN +"</th>";
                      html+="</tr>";
                      html+="</table>";
                     ...
                     result:function(json,item){
                         ...
                         for( var i = -1, y = item.length; ++i != y;){ 
                                 ...
                                 html[j++]="<td style='text-align: left;'>" + item[i].SONGAYDIEUTRI + "</td>";
                                 html[j++]="<td style='text-align: left;'>" + item[i].CHUYENDENBENHVIEN + "</td>";
                                 html[j++]="</tr>";
                         }
...
{
  $.BindFind({
        ajax: "../ajax/zBaoCaoBNRaVienChuyenVien_ajax1.aspx?do=setDefault"
                , useEnabled: false
    }, null, function () {
        }
    ); 
...
{
    $(control).TimKiem({
                     ajax:"../ajax/zBaoCaoBNRaVienChuyenVien_ajax1.aspx?do=XuatExcel&idkhoa="+$.mkv.queryString("IdKhoa")+"",dataType:'text',showPopup:false
               },function () {
                    $.mkv.loading();
                    return true;
               },function (data) {
