 ...
 function xoa(control,bool){
  ...
  function Find(control) {
      $(control).TimKiem({
             ajax:"../ajax/tinh_ajax2.aspx?do=TimKiem",dataType:'json',showPopup:false
       },function () {
            ...
            $("#tableAjax_tinh").html(html);
      }).myfilter({
           txtfilter:"#gridTable [name=search]",
           paging:function(json,data){
                $("#tableAjax_tinh").find("#paging").html(data);
           },
           result:function(json,item){
                ...
                for( var i = -1, y = item.length; ++i != y;){ 
                      ...
                      html[j++]="<td><input mkv='true' id='tinhname' type='text' value='" + item[i].TINHNAME + "' style='width:100%' " + (!edit ? "disabled" : "") + "/></td>";
                      html[j++]="<td><input mkv='true' id='MaTinh' type='text' value='" + item[i].MATINH + "' style='width:100%' " + (!edit ? "disabled" : "") + "/></td>";
                      html[j++]="<td><input mkv='true' id=\"idkhoachinh\" mkv=\"true\" type=\"hidden\" value='" + item[i].TINHID + "'/></td>";
                      html[j++]="</tr>";
                }
