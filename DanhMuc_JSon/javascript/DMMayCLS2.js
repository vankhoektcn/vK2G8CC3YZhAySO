 $(document).ready(function() {
     $('input[id^=luuTable]').click(function () {
        $(this).LuuTable({ajax:"../ajax/DMMayCLS_ajax2.aspx?do=luuTable",tablename:"gridTable"});
     });
 ...
 function xoa(control,bool){
    if(bool || bool == null)
      $(control).XoaRow({ajax:'../ajax/DMMayCLS_ajax2.aspx?do=xoa'});
 }
  function Find(control) {
      $(control).TimKiem({
             ajax:"../ajax/DMMayCLS_ajax2.aspx?do=TimKiem",dataType:'json',showPopup:false
       },function () {
             $("#tableAjax_DMMayCLS").html('<img src="../images/loading.gif" style="margin:0 41%;padding:10px 0 10px 0"/>'); 
             return true;
       },function (data) {
            var html = "";
            ...
            $("#tableAjax_DMMayCLS").html(html);
      }).myfilter({
           txtfilter:"#gridTable [name=search]",
           paging:function(json,data){
                $("#tableAjax_DMMayCLS").find("#paging").html(data);
           },
           result:function(json,item){
                ...
                for( var i = -1, y = item.length; ++i != y;){ 
                      ...
                      html[j++]="<td><input mkv='true' id='name' type='text' value='" + item[i].NAME + "' style='width:100%' " + (!edit ? "disabled" : "") + "/></td>";
                      html[j++]="<td><input mkv='true' id='maloai' type='text' value='" + item[i].MALOAI + "' style='width:100%' " + (!edit ? "disabled" : "") + "/></td>";
