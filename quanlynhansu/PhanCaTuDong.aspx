<%@ Page Language="C#" AutoEventWireup="true" CodeFile="PhanCaTuDong.aspx.cs" Inherits="PhanCaTuDong" %>
...
var dp_cal, dp_cal1,SoGioTangCa;      
	window.onload = function () 
	{
	    ...
	    //saveasThietLapCa("1", "1,2", "15/09/2011","15/09/2011");
	};
...
function setChonCaPhong(listIdNVbyPB,idphong,idCheck)
{
 ...
 var strIDDV = document.getElementById('listIdNV').value; 
 var ListNhanVienId=listIdNVbyPB;
...
function LuuThietLapCa(update)
{
    var idTangCaKiemTra = document.getElementById("txtidTangCa").value;
     ...
     //End Kiem tra Gio
//Kiểm tra dữ liệu 
    var idCaLamViec = document.getElementById("ddlLoaiTangCa").value;
    ...
    {
        alert("Bạn chưa chọn ngày phân ca!");
        document.getElementById('txtNgay').focus();
        return false;
    }
    var txtDenNgay = document.getElementById("txtDenNgay").value;
    if(txtDenNgay=="0" || txtDenNgay=="")
    {
        alert("Bạn chưa chọn khoảng ngày phân ca!");
        document.getElementById('txtDenNgay').focus();
        return false;
    }
    //Luu Thiết lập ca
     ...
     if(update==1)
     {
        if(idTangCa=="")
        {
            alert("Bạn chưa chọn Ca muốn cập nhật");
            return false;
        }
        //UpdateThietLapCa(idTangCa,idCaLamViec,listIDNV,txtNgay,ddlTuGioText,ddlDenGioText,this.SoGioTangCa);
        UpdateThietLapCa(idCaLamViec,listIDNV,txtNgay,txtDenNgay);
     }
     else
        //saveasThietLapCa(idCaLamViec,listIDNV,txtNgay,ddlTuGioText,ddlDenGioText,this.SoGioTangCa);
        saveasThietLapCa(idCaLamViec,listIDNV,txtNgay,txtDenNgay);
     
}
function saveasThietLapCa(idCaLamViec, listIDNV, txtNgay,txtDenNgay)
	{
	    xmlHttp = GetMSXmlHttp();
        ...
        xmlHttp.open("GET","ajax.aspx?do=LuuPhanCaTuDong&idCaLamViec="+idCaLamViec+"&listIDNV="+listIDNV+"&txtNgay="+txtNgay+"&txtDenNgay="+txtDenNgay+"&times="+Math.random() ,true);
        xmlHttp.send(null);
	}
function UpdateThietLapCa(idCaLamViec, listIDNV, txtNgay,txtDenNgay)
{
    xmlHttp = GetMSXmlHttp();
        ...
        xmlHttp.onreadystatechange = function()
        {
            if(xmlHttp.readyState == 4)
            {
                ...
                if(eval(value) == 0)
                {
                   alert("Có lỗi xảy ra trong quá trình cập nhật thông tin phân ca. Vui lòng thử lại lần sau.");
                   return false;
                }               
                else
                {   
                    alert("Đã cập nhật phân ca thành công !");    
                    //document.getElementById('listIdNV').value="";
                    //document.getElementById("showDanhSachNVchon").style.display = 'none';
                } 
            }
        }
        xmlHttp.open("GET","ajax.aspx?do=CapNhatPhanCaTuDong&idCaLamViec="+idCaLamViec+"&listIDNV="+listIDNV+"&txtNgay="+txtNgay+"&txtDenNgay="+txtDenNgay+"&times="+Math.random() ,true);
        xmlHttp.send(null);
}
function LoadThoiGianCa()
{
    var idcalamviec=document.getElementById('ddlLoaiTangCa').value;
    if(idcalamviec=="0")
    {
        document.getElementById('txtTuGio').value="";
        document.getElementById('txtDenGio').value="";
        return;
    }
    xmlHttp = GetMSXmlHttp();
        ...
        xmlHttp.onreadystatechange = function()
        {
            if(xmlHttp.readyState == 4)
            {
               ...
               if(value == "0")
                {
                   alert("Có lỗi xảy ra trong quá trình lấy thông tin ca. Vui lòng thử lại lần sau.");
                   return false;
                }               
                else
                {   
                    var arrThoiGian = value.split(',');
                    document.getElementById('txtTuGio').value=arrThoiGian[0];
                    document.getElementById('txtDenGio').value=arrThoiGian[1];
                } 
            }
        }
        xmlHttp.open("GET","ajax.aspx?do=LoadThoiGianCa&idcalamviec="+idcalamviec+"&times="+Math.random() ,true);
        xmlHttp.send(null);
}

function SetValueEmpty()
{
    window.location.href="PhanCaTuDong.aspx";
    return;
    document.getElementById('listIdNV').value="";
    document.getElementById("showDanhSachNVchon").style.display = 'none';
    document.getElementById('txtidTangCa').value="";
    document.getElementById('ddlLoaiTangCa').value="0";
    document.getElementById('txtTuGio').value="";
    document.getElementById('txtDenGio').value="";
    document.getElementById('txtNgay').value=document.getElementById('txtNgayHienTai').value;
    document.getElementById('txtDenNgay').value=document.getElementById('txtNgayHienTai').value;
    showKQCLS();
}
</script>
    ...
    <div style="background-color: White;" align="left">
        <div style="width: 100%; padding-left: 10px; background-color: #FBF8F1; height: 19px;
            ...
            <span style="color: Red; font-weight: bold; font-size: 12pt;">
                <div style="margin-top: 0px; padding-left: 5px">
                    PHÂN CA TỰ ĐÔNG(hành chính)</div>
            </span>
        </div>
        <br />
        <div style="text-align: left; width: 700px; padding-left: 50px; background-color: White;">
            <table width="700px" rules="none" cellpadding="0px" cellspacing="0px" style="border-color: Blue;
                border-style: double; border: 4px; padding-left: 0px;" bgcolor="#a7a9b8">
                <%--<tr>
                ...
                <tr>
                    ...
                    <td valign="top" align="left" style="width: 25%; height: 40px; padding-right: 0px">
                        <p class="ptext">
                            ...
                            <asp:dropdownlist id="ddlLoaiTangCa" onchange="LoadThoiGianCa();" runat="server" width="100%" tabindex="1"></asp:dropdownlist>
                            &nbsp;</p>
                    </td>
                ...
                <tr>
                    <td colspan="4" style="height: 60px">
                        <table width="100%" cellpadding="0px" cellspacing="0px" style="padding-left: 0px;
                            padding-right: 0px">
                            <tr>
                                <td valign="top" align="left" style="height: 40px; padding-left: 10px; width: 5%;">
                                    Ngày:
                                </td>
                                <td valign="top" align="right" style="width: 41%; height: 40px; padding-left: 10px">
                                    <asp:textbox id="txtNgay" runat="server" width="27%" tabindex="5"></asp:textbox>
                                    <input id="Button2" type="button" value="..." onclick="dp_cal.toggle()" />&nbsp;đến
                                    ngày
                                    <asp:textbox id="txtDenNgay" runat="server" tabindex="5" width="27%"></asp:textbox>
                                    <input id="Button1" type="button" value="..." onclick="dp_cal1.toggle()" /></td>
                                <td valign="top" align="right" style="height: 40px; padding-left: 10px; width: 8%;">
                                    Từ :&nbsp;</td>
                                <td valign="top" align="left" style="width: 22%; height: 40px; padding-right: 0px">
                                    <%--<asp:textbox id="txtTuGio" runat="server" Width="95px"></asp:textbox>--%>
                                    <input id="txtTuGio" type="text" runat="server" style="width: 70px" readonly="readOnly" />
                                    (hh:mm)
                                ...
                                <td valign="top" align="right" style="padding-right: 2px; width: 3%; height: 40px;">
                                    Đến :
                                </td>
                                <td valign="top" align="right" style="width: 25%; height: 40px; padding-right: 0px">
                                    ...
                                    <input id="txtDenGio" type="text" runat="server"  style="width: 68px" readonly="readOnly" />
        ...
        <div style="text-align: center; width: 900px">
            ...
            <input type="hidden" value="" runat="server" name="txtNgayHienTai" id="txtNgayHienTai" />
        </div>
        <div style="width: 100%; text-align: left; padding-left: 300px">
            ...
            <asp:button id="btnUpDate" runat="server" onclientclick="return LuuThietLapCa(1)"
                text="Cập nhật" visible="false" width="92px"></asp:button>
            ...
            <input id="Button5" style="width: 50px; cursor: pointer" type="button" value="Mới"
                ...
                <asp:button id="btnTim" runat="server" 
                text="Tìm"  width="50px" OnClick="btnTim_Click"></asp:button>
        </div>
                ...
                <tr>
                    <td valign="top" align="center" colspan="2" height="20" style="width: 100%">
                        <asp:datagrid id="dgr" tabindex="12" runat="server" width="100%" allowsorting="True"
                            autogeneratecolumns="False" datakeyfield="idcalamviec" borderwidth="1px" bordercolor="Silver"
...
<asp:TemplateColumn><ItemTemplate>
				...
				<asp:Label runat="server" id="lbNgayLamca" visible="false" Text='<%#Bind("Ngay") %>'></asp:Label>									    
</ItemTemplate>

<HeaderStyle Width="7%"></HeaderStyle>
</asp:TemplateColumn>
<asp:BoundColumn DataField="tencalamviec" HeaderText="Tên ca">
...
<%--<asp:BoundColumn DataField="sogiotangca" HeaderText="Số giờ ca">
