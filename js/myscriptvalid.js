...
//───────────────────────────────────────────────────────────────────────────────────────
function formatCurrencyOption(num,phanngan,thapphan,laysoLe) 
 {
    var sodautien= num;
    num = num.toString().replace(/\$|\,/g,'');
    if(isNaN(num))
    ...
    for (var i = 0; i < Math.floor((num.length-(1+i))/3); i++)
    {
        num = num.substring(0,num.length-(4*i+3))+ phanngan +
        num.substring(num.length-(4*i+3));
    }
    return (((sign)?'':'-') + num + (laysoLe ? thapphan + nvkSoLe:""));
}
function TestDate(t) {
    var ngay = t;
    if (t.value.length > 0) {
