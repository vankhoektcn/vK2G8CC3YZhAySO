...
//   {        
        ...
        div.style.width = (wid!='')?wid:'auto';
        if(hei!='')div.style.height = hei + "px";
...
function createTip_New(td,divname,divchildname,title,ajaxname,sloganajax,error,url, wid, hei,functionclose)
{ 
    try
   {        
        hideTip(divname);                
        ...
        td.appendChild(div);        
        createTable_New(divname,divchildname,title,ajaxname,sloganajax,error,url, wid, hei,functionclose);
    }
    catch(exception)
   {
   }
...
//==== createtable moi vanduc
function createTable_New(divname,divchildname,title,ajaxname,sloganajax,error,url, wid, hei,functionclose)
{
    try
    {
        var div = document.getElementById(divname);
        ...
        td.style.paddingRight = "0px";
        td.style.height = "20px";        
        td.innerHTML = "<span style=\"float:left;width:280px;padding-top:0px;cursor:pointer;\" onmousedown=\"mouseDown('"+divname+"')\" onmouseup=\"mouseRelease()\">"+title+"</span><span style=\"float:right\"><img src=\"../images/close.gif\" align=\"right\"/ title=\"click vào để đóng danh sách\" onclick=\""+functionclose+"('"+divname+"')\" style=\"cursor:pointer;\"></span>";        
        div.appendChild(table);        
        tr = table.insertRow(1);
        td = tr.insertCell(0);        
        td.style.width="350px";
...
function loadDanhSachKho(makho)
{
    try
    {
        var span = document.getElementById("ajaxfrm_DM_Kho");
        span.style.display = "";
...
function loadChiTietKho(makho)
{
    ...
    xmlHttp.onreadystatechange = function()
    {
        if(xmlHttp.readyState == 4)
        {
            ...
            var ajax = document.getElementById("ajaxfrm_DM_Kho");
            ajax.style.display = "";
            if(value == "error")
            {
                alert("Lỗi xảy ra trong quá trình truyền dữ liệu");
...
function newKho()
{
    try
    {
        ...
        html += "<span class=\"ajax\" id=\"ajaxfrm_DM_Kho\" style=\"display:none;\"><img src=\"images/processing.gif\" border=\"0\" />&nbsp;đang load dữ liệu ...</span>";
        td.innerHTML = html;
    }
    catch(exception)
    {
...
function savenewhangsanxuat()
{
    try
    {
        ...
        if(!NumAndChar.test(makho.value))
        {
            alert("Mã Nước SX chỉ cho phép nhập chữ hoặc số . Vui lòng nhập lại");
            makho.focus();
            return false;
        }
        var tenkho = document.getElementsByName("tenkho")[0];
        if(trim(tenkho.value) == "")
        {
            alert("Tên Nước SX không được để trống . Vui lòng nhập lại ");
            tenkho.focus();
            return false;
        }
        var tenkho = document.getElementsByName("tenkho")[0];
...
function savenewnhasanxuat()
{
    try
    {
        ...
        if(!NumAndChar.test(makho.value))
        {
            alert("Mã nhà sản xuất chỉ cho phép nhập chữ hoặc số . Vui lòng nhập lại");
            makho.focus();
            return false;
        }
        var tenkho = document.getElementsByName("tenkho")[0];
        if(trim(tenkho.value) == "")
        {
            alert("Tên nhà sản xuất không được để trống . Vui lòng nhập lại ");
            tenkho.focus();
            return false;
        }
        var tenkho = document.getElementsByName("tenkho")[0];
        ...
        var ajax = document.getElementById("ajaxkho");
        ajax.style.display = "";
        loadCheckMaNhaSanXuat(makho.value);
    }
    catch(exception)
    {
        alert("Có lỗi xảy ra . Vui lòng xem lại script ");
...
function loadCheckMaHangSanXuat(mahang)
{
    ...
    xmlHttp.onreadystatechange = function()
    {
        if(xmlHttp.readyState == 4)
        {
            ...
            {
                alert("Mã Nước SX này đã sử dụng rồi . Vui lòng chọn mã khác");
                ajax.style.display = "none";
                return;
            }
            else if(value == 2)
...
function loadCheckMaNhaSanXuat(mahang)
{
    xmlHttp = GetMSXmlHttp();
    xmlHttp.onreadystatechange = function()
    {
        if(xmlHttp.readyState == 4)
        {
            ...
            else if(value == 2)
            {
                var f = document.getElementById("kho");
                var sc = document.getElementById("secondtime");
                sc.value = "newnhasanxuat";
                //f.secondtime.value = "newhangsanxuat";
                f.submit();
            }
        }
    }
    xmlHttp.open("GET","ajax.aspx?do=checkmanhafromhansanxuat&mahang="+mahang+"&times="+Math.random(),true);
    xmlHttp.send(null);
}
//===========================================
...
function loadChiTietNhaSanXuat(mahang)
{
    xmlHttp = GetMSXmlHttp();
    xmlHttp.onreadystatechange = function()
    {
    ...
    xmlHttp.open("GET","ajax.aspx?do=loadchitietnhasanxuat&mahang="+mahang+"&times="+Math.random(),true);
    xmlHttp.send(null);
}
//===========================================
function newHang()
...
function newNha()
{
    try
    {
        ...
        var td = document.getElementById("functionkho");
        html = "<br/>";
        html += "<input type=\"button\" name=\"luu\" onclick=\"savenewnhasanxuat()\" value=\"Lưu\" style=\"width:80px;\"/>&nbsp;&nbsp;";
        html += "<input type=\"reset\" name=\"cancel\" value=\"Làm lại\" style=\"width:80px;\"/>&nbsp;&nbsp;";
        html += "<input type=\"button\" name=\"xoa\" value =\"Xóa\" onclick=\"deletenhasanxuat('checkall')\" style=\"width:80px;\" />&nbsp;&nbsp;";
        html += "<span class=\"ajax\" id=\"ajaxfrm_DM_Kho\" style=\"display:none;\"><img src=\"images/processing.gif\" border=\"0\" />&nbsp;đang load dữ liệu ...</span>";
        td.innerHTML = html;
    }
    catch(exception)
...
function deletehangsanxuat(checkname)
{
    ...
    if(iCount == 0)
    {
        alert("Vui lòng chọn ít nhất một Nước SX trước khi xóa ");
        return false;
    }
    else if(confirm("Bạn có chắc muốn xóa những gì bạn chọn không ?"))
    {
...
function deletenhasanxuat(checkname)
{
    var iCount = checkItemChecked(checkname);
    if(iCount == 0)
    {
        alert("Vui lòng chọn ít nhất một nhà sản xuất trước khi xóa ");
        return false;
    }
    else if(confirm("Bạn có chắc muốn xóa những gì bạn chọn không ?"))
    {
        var f = document.kho;
        f.secondtime.value = "deletenhasanxuat";
        f.submit();
    }
}
//===========================================
...
function saveupdatenhasanxuat()
{
    try
    {
        ...
        var f = document.kho;
        f.secondtime.value = "updatenhasanxuat";
        f.submit();
    }
    catch(exception)
    {
...
function savenewvattu()
{
    try
    {
        ...
        if(hangsx.value == "0")
        {
            alert("Bạn chưa chọn Nước SX . Vui lòng nhập lại ");
            hangsx.focus();
            return false;
        }
    ...
    xmlHttp.onreadystatechange = function()
    {
        if(xmlHttp.readyState == 4)
        {
            ...
            else if(value >= 0)
            {
                ...
                arr[1] = "frm_DS_PhieuXuatKho.aspx";
                document.location.href = arr[value];                
            }
        }
    }
...
//            var otkvat = document.getElementById("txttkvat");
            ...
            if (opxid.value == "-1")
            {
                ...
                var oidtu = document.getElementById("cboTuThuoc");
                var oidngan = document.getElementById("cboNgaThuoc");
            ...
            {
                if ( CheckThongTinPhieuNhap(oSoCt, oNgayCt, oKho, oNCC, ongayhoadon, otennguoigiao, oDienGiai, oidtu, oidngan) && CheckThongTinCTPhieuNhap(omavt,osoluong,odongia, olosx, ongayhethan))
                {
//                    alert("id tu: " + oidtu.value);
//                    alert("id ngan: " + oidngan.value);
                    LuuPhieuNhap(oSoCt.value, oNgayCt.value, oKho.value, oDienGiai.value, oNCC.value, okyhieuhd.value, osohoadon.value, ongayhoadon.value, otennguoigiao.value, onhapcho.value, omavt.value,osoluong.value,odongia.value, ochietkhau.value, ovat.value, olosx.value, ongayhethan.value, oidtu.options[oidtu.selectedIndex].value, oidngan.options[oidngan.selectedIndex].value);
                }
            }
            else
            {
               if( CheckThongTinCTPhieuNhap(omavt,osoluong,odongia, olosx, ongayhethan))
               {
                     ...
                     LuuChiTietPhieuNhap(omavt.value,osoluong.value,odongia.value,ochietkhau.value,ovat.value,olosx.value, ongayhethan.value, oidtu.value, oidngan.value);
               }
            }
	    }  
	    return;
    ...
    function CheckThongTinPhieuNhap(oSoCt, oNgayCt, oKho, oNCC, oNgayHoaDon, oTenNguoiGiao, oDienGiai, oidtu, oidngan)
    {
        if (oSoCt.value == "")
        {
        ...
        if(oidtu.value == "" || oidtu.value == "0")
        {
            alert("Bạn chưa chọn tủ thuốc. Vui lòng kiểm tra lại.");
            oidtu.focus();
            return false;
        }
        if(oidngan.value == "" || oidngan.value == "0")
        {
            alert("Bạn chưa chọn ngăn thuốc. Vui lòng kiểm tra lại.");
            oidngan.focus();
            return false;
        }
        return true;
    ...
    function LuuPhieuNhap(oSoCt, oNgayCt, oKho, oDienGiai, oNCC, oKyHieuHoaDon, oSoHoaDon, oNgayHoaDon, oTenNguoiGiao, oNhapCho, omavt, osoluong, odongia, ochietkhau, ovat, olosx, ongayhethan, oidtu, oidngan)
    {
        //alert(oSoCt+ " " + oNgayCt+ " " + oKho+ " " + oDienGiai+ " " + oNCC+ " " + oNgayHoaDon+ " " + oTenNguoiGiao+ " " + omavt+ " " + osoluong+ " " + odongia+ " " + olosx+ " " + ongayhethan);
        xmlHttp = GetMSXmlHttp();
//        alert("idtu = " + oidtu);
//        alert("idngan = " + oidngan);
        xmlHttp.onreadystatechange = function()
        {
            if(xmlHttp.readyState == 4)
            {
	...
	function CheckNewPX()
    {
        ...
        xmlHttp.onreadystatechange = function()
        {
            if(xmlHttp.readyState == 4)
            {
                ...
                {
                    ResetSession();
                    document.location.href = "frm_PhieuXuatKho_View.aspx";
                }                
            }
        }
