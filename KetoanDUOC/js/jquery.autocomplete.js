...
$.fn.extend({
	...
	firstdata:function (options) {
	    this.focus();
	    return this.trigger("getFirst");
	}
});
...
$.Autocompleter = function(input, options) {
	...
	}).bind("getFirst", function() {
	   	$.ajax({
				type:"GET",
				url: options.url,
				success: function(data) {
				    $("#"+$input.attr("id").split('mkv_')[1]).val(data.split('\r\n')[0].split('|')[1]);
...
$.Autocompleter.Select = function (options, input, select, config) {
	...
	return {
		...
		hide: function() {
			element && element.animate({ height: 'hide', left: '-50', top: '-50', opacity: 'hide' }, 'slow');
