	...
	function selectCurrent() {
		...
		if($input.parents("table").attr("id") == null){
	        if($("#delete_result_"+$input.attr("id")+"").attr("id") == null)
		        $input.after($("<img style='cursor:pointer;float:right;border:0;position:absolute' id='delete_result_"+$input.attr("id")+"' onclick=\"javascript:"+options.triggerDelete+";$('#"+$input.attr("id")+"').val('');$('#"+$input.attr("id").replace('mkv_','')+"').val('');$(this).remove();\" src='../images/delete_icon.png'/>"));
		}else{
		        
		    if ($input.parents("table").find("tr").eq($($input).parent().parent().index() + 1).find("input:first").length < 1){
                             if(options.addRow == true)
                                $.mkv.themDongTable($input.parents("table").attr("id"));
		    ...
		    if($input.parents("table").find("tr").eq($input.parent().parent().index()).find("#delete_result_"+$input.parents("table").attr("id")+"_"+$input.attr("id")+"").attr("id") == null)
		        $input.after($("<img style='cursor:pointer;float:right;border:0;position:absolute;width:10px;' id='delete_result_"+$input.parents("table").attr("id")+"_"+$input.attr("id")+"' onclick=\"javascript:"+options.triggerDelete+";$(this).parents('table').find('tr').eq($(this).parent().parent().index()).find('#"+$input.attr("id")+"').val('');$(this).parents('table').find('tr').eq($(this).parent().parent().index()).find('#"+$input.attr("id").replace('mkv_','')+"').val('');$(this).remove();\" src='../images/delete_icon.png'/>"));
		}
...
$.Autocompleter.Select = function (options, input, select, config) {
	...
	function init() {
		...
		if(options.header != "")
		{
		   $(".ac_results").css({'padding-top':(($(options.header).height() > 0)?($(options.header).height()+1):13)+"px"});
	       $("<div style='position:absolute;top:0;left:0;padding:2px 0 2px 0;background-color:#648CCC;text-align:center;border-bottom:1px solid #CCC;border-left:1px solid #cfcfcf;color:white;width:100%;'>"+options.header+"</div>").appendTo(element);
	    }
	    else
