	...
	function selectCurrent() {
		...
		if($input.parents("table").attr("id") == null){
		    ...
		    if ($input.parents("table").find("tr").eq($($input).parent().parent().index() + 1).find("input:first").length < 1){
                ...
                if($.browser.msie){       
                    var hamtimkiem = $input.parents("table").find("tr:eq(1)").find("#"+$input.attr("id")).attr("onfocus"); 
                    $input.parents("table").find("tr").eq($($input).parent().parent().index() + 1).find("#"+$input.attr("id")).attr("onfocus",hamtimkiem);
                }
            }
