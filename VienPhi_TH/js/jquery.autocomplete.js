...
; (function ($) {
    $.fn.extend({
        ...
        abc: function (options) {
            if ($(this).prevAll('input[type=text]:first').is(":disabled")) return;
            $(this).prevAll('input[type=text]:first').focus();
            hostabc = true;
            return $(this).prevAll('input[type=text]:first').trigger("getAll");
        }
    });
    $.Autocompleter = function (input, options) {
        ...
        $input.bind(($.browser.opera ? "keypress" : "keydown") + ".autocomplete", function (event) {
            ...
            if ($.trim($input.val()).length < 1) {
                if ($input.attr("id").indexOf("mkv_") != -1)
                    $(this).prevAll('input[type=hidden]:first:[id=' + $input.attr("id").replace('mkv_', '') + ']').val('');
            }
            hasFocus = 0;
        ...
        function selectCurrent() {
            ...
            $input.val(v);
            if ($input.attr("id").indexOf("mkv_") != -1)
                $input.prevAll('input[type=hidden]:first:[id=' + $input.attr("id").replace('mkv_', '') + ']').val(options.formatHiddenValue(selected.data));
            hideResultsNow();
    ...
    $.Autocompleter.defaults = {
        ...
        cacheLength: 10,
        max: 100,
        formatHiddenValue: function (row) { return row[1]; },
        header: "",
        mustMatch: false,
        extraParams: {},
        selectFirst: true,
    ...
    $.Autocompleter.Select = function (options, input, select, config) {
		...
		.appendTo(document.body);
            ...
            if (options.header != "") {
                ...
                $("<div class='header'>" + options.header + "</div>").appendTo(element);
            }
            else
