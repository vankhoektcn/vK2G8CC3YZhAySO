$(document).ready(function(){
    ...
    $("#LayDS").click(function(){
        $.ajax({
            ...
            dataType:"text",
            type:"GET",
            url:"ajax/TraTienKham_CLS_ajax.aspx?do=loadDSBNCLS&ngaybd=" + $("#ngaybd").val() + "&mabenhnhan=" +$("#mabenhnhan").val() + "&tenbenhnhan=" + $("#tenbenhnhan").val()
                        ,
...
function HienThiChiTietCLS(control)
{
    ...
    $(control).TimKiem({
        ajax:"ajax/TraTienKham_CLS_ajax.aspx?do=HienThiChiTietCLS&MaPhieu=" + MaPhieu
         ,title:"Danh sách dịch vụ CLS - " + MaPhieu,height:"300px",width:"1050px"
    });
}
function setHoanTraCLS(control){
    ...
    $.ajax({
        ...
        cache:false,
        type:"POST",
        url:"ajax/TraTienKham_CLS_ajax.aspx?do=setHoanTraCLS&MaPhieu=" + $("#maphieu").val() + "&ngayhoantra=" + $("#ngayhoantra").val() + "&lstId="+lst
        ,
        success:function(value){
            $.mkv.myalert(value,2000,"success");
             window.open("frm_rpt_InPhieuThuTH.aspx?MaPhieu=" + $("#maphieu").val() + "&intructiep=0&isdahuy=1" + "&ngayhoantra=" + $("#ngayhoantra").val(),"_blank");
            $.mkv.closeDivTimKiem();
            $("#LayDS").click();
        },
        error:function(msg){
