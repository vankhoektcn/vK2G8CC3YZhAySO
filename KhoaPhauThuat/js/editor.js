//
// htmlArea v2.03 - Copyright (c) 2002 interactivetools.com, inc..// This copyright notice MUST stay intact for use (see license.txt)..//
// A free WYSIWYG editor replacement for <textarea> fields..// For full source code and docs, visit http://www.interactivetools.com/
//

// write out styles for UI buttons
document.write('<style type="text/css">\n');
document.write('.btn     { width: 22px; height: 22px; border: 1px solid buttonface; margin: 0; padding: 0;background-color: ; }\n');
document.write('.btnOver { width: 22px; height: 22px; border: #0A246A 1px solid; background-color: #B6BDD2; }\n');
...
this.btnList = {
    ...
    "custom2":           ['custom2',         'Purpose of button 2',  'editor_action(this.id)',  'justify.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
	  ...
	  // Dinh dang
    if (btnName == "dinhdang") {
        toolbar += '<select id="_' +objname+ '_DinhDang" onChange="editor_action(this.id)" unselectable="on" style="margin: 1 2 0 2; font-size: 12px;">';
        for (var dinhdang in config.dinhdangs) {
          toolbar += '<option value="' +config.dinhdangs[dinhdang]+ '">' +dinhdang+ '</option>'
        }
        toolbar += '</select>';
        continue;
      }
...
function editor_action(button_id) {
	var d=new Date();
	var stime=d.getHours()+"."+d.getMinutes()+"."+d.getSeconds()+"."+d.getDate()+"."+d.getMonth()+"."+d.getFullYear(); 
  ...
  // help popup
  if (cmdID == 'showhelp') {
    window.open(_editor_url + "popups/editor_help.html?s="+stime, 'EditorHelp');
    return;
  }

  // popup editor
  if (cmdID == 'popupeditor') {
    window.open(_editor_url + "popups/fullscreen.html?"+objname+"&stime="+stime,
                'FullScreen',
  ...
  // FontStyle (change CSS className)
    ...
    editdoc.execCommand('DinhDang',0,'636c6173734e616d6520706c616365686f6c646572');
  ...
  // fgColor and bgColor
    ...
    var newcolor = showModalDialog(_editor_url + "popups/select_color.html?stime="+stime, oldcolor, "resizable: no; help: no; status: no; scroll: no;");
  ...
  // commandID and pass it to execCommand().   See http://msdn.microsoft.com/workshop/author/dhtml/reference/commandids.asp
    ...
    // insert image
		...
		showModalDialog(_editor_url + "popups/insert_image.html?stime="+stime, editdoc, "resizable: no; help: no; status: no; scroll: no; ");
      //showModalDialog(_editor_url + "popups/insert_image.html?stime="+stime, editdoc, "resizable: no; help: no; status: no; scroll: no; ");
    }

    // insert table
    else if (cmdID.toLowerCase() == 'inserttable'){
      showModalDialog(_editor_url + "popups/insert_table.html?"+objname+"&stime="+stime,
                                 window,
                                 "resizable: yes; help: no; status: no; scroll: no; ");
    }
...
function editor_updateToolbar(objname,action) {
  ...
  if (action == "enable" || action == "disable") {
    var tbItems = new Array('FontName','FontSize','DinhDang','FontStyle');                           // add pulldowns
  ...
  //Dinhdang
 var dinhdang_obj = document.all["_" +objname+ "_DinhDang"];
  if (dinhdang_obj) {
    var dinhdang = editdoc.queryCommandValue('DinhDang');
    if (dinhdang == null) { dinhdang_obj.value = null; }
    else {
      var found = 0;
      for (i=0; i<dinhdang_obj.length; i++) {
        if (dinhdang.toLowerCase() == dinhdang_obj[i].text.toLowerCase()) {
          dinhdang_obj.selectedIndex = i;
          found = 1;
        }
      }
      if (found != 1) { dinhdang_obj.value = null; }     // for fonts not in list
    }
  }
