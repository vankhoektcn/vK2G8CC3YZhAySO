<%@ Page Language="C#" AutoEventWireup="true" CodeFile="HDDV_DanhSachHoaDon_VanTam.aspx.cs" Inherits="ketoan_HDDV_DanhSachHoaDon" %>
<%@ Register Src="~/ketoan/Menu_KT/uscmenuKT_HoadonDV.ascx" TagName="menu_ketoanhoadonDV" TagPrefix="uc1" %>

<!--#include file = "header.htm" -->

<%--<%@ Register Src="menu_ketoanhoadondichvu.ascx" TagName="hoadondichvu" TagPrefix="uc2" %>--%>
...
{      
    ...
    ResetTableDSPhieuThu();
     ...
     xmlHttp = GetMSXmlHttp();
        xmlHttp.onreadystatechange = function()
        {
          ...
          xmlHttp.open("GET","ajax.aspx?do=DanhSachHoaDon&TuNgay="+TuNgay+"&DenNgay="+DenNgay+"&MaKH="+MaKH+"&SoHD="+SoHD+"&loaixuat="+loaixuat+"&times="+Math.random(),true);
        xmlHttp.send(null);
 }
 ...
 {//hoa_donid,so_hd,convert(varchar,ngay_lap_hd,103)ngay_lap_hd, ma_kh,dien_giai,tk_no,tk_co,tk_thue,thue_suat,tien,tien_thue,tong_tien                
    ...
    var shtml = "<tr class=\"RowGidView\">";        
        ...
        if('<%=StaticData.HavePermision(this.Page, "KeToanHDDV_HDDV_DanhSachHoaDon_Xoa")%>' == 'False')
            shtml += "		<td style=\"width:50px;text-align:center\"><label id=\"Xoa_"+(lastRow-1)+"\" style=\"cursor:pointer;font-style:italic;color:Blue\" onclick=\"XoaHoaDon('"+SoHD+"','"+NgayLapHD+"')\"></label></td>	";
        else
        ...
        shtml += "		<td style=\"width:100px;text-align:center\"><a href=\""+link+"?idHoaDon="+HD_id+"&SoHD="+SoHD+"&loai_hd="+loai_hd+"&MaKH="+MaKH+"\"><input type=\"hidden\" id=\"hdSoHD_"+(lastRow-1)+"\" value=\""+SoHD+"\"/>"+SoHD+"</a></td>	";
...
//kiem tra hoa don truoc khi xoa; hoa don nao da thu thi se ko dc xoa(muon xoa thi phai xoa phieu thu truoc)
function kiemtraxoahd(sohd,ngaylaphd)
{        
     var xmlHttp = GetMSXmlHttp();
        ...
        xmlHttp.onreadystatechange = function()
        {
            if(xmlHttp.readyState == 4)
            {
                ...
                if(value=="1")
                {                    
                    alert('Hóa đơn này đã lập phiếu thu rồi, không được xóa!');                    
                }
                else
                {
                    getFunctionXoaHoaDon(sohd,ngaylaphd);
        ...
        xmlHttp.open("GET","ajax.aspx?do=kiemtraxoasuahoadon&SoHD="+sohd+"&NgayLapHD="+ngaylaphd+"&times="+Math.random(),false);
        xmlHttp.send(null); 
}
...
{      
      var xmlHttp = GetMSXmlHttp();
        ...
        xmlHttp.open("GET","ajax.aspx?do=XoaHoaDonBanHang&SoHD="+SoHD+"&NgayLapHD="+NgayLapHD+"&times="+Math.random(),false);
        xmlHttp.send(null);
}
...
<div width = "100%" align = "left" style="height:  34px;background-color:#007138"> 
    <uc1:menu_ketoanhoadonDV ID="hoadondichvu1" runat="server" /></div>
<div>
    <table>
        <tr>
            <td>Từ ngày : </td>
