...
public partial class KTCN_NhaCungCap_TH : System.Web.UI.Page
{
    ...
    void loadtk()
    {
        string sql = "select taikhoan,tentaikhoan from danhmuctk where taikhoan like '331%' or taikhoan like '338%'  order by taikhoan";
        DataTable dt = DataAcess.Connect.GetTable(sql);
        StaticData.FillCombo(drltkcn1, dt, "taikhoan", "taikhoan");
    ...
    private DataTable LoadCongNoKhachHang_TH(string strTK)
    {
        ...
        dtSRC = null;
        sql = "exec spCong_No_Nha_Cung_Cap_TH  '"+ngay_dau_nam+"','"+ StaticData.CheckDate_kt(txtNgay.Text) + "','" + StaticData.CheckDate_kt(txtDenNgay.Text) + "','" + strTK + "'";
