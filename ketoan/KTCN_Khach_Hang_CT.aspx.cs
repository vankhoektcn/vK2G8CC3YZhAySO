...
public partial class KTCN_KhachHang_CT : System.Web.UI.Page
{
    ...
    void loadtk()
    {
        string sql = "select taikhoan,tentaikhoan from danhmuctk where taikhoan like '131%' or taikhoan like '138%' or taikhoan like '141%' order by taikhoan";
        DataTable dt = DataAcess.Connect.GetTable(sql);
        StaticData.FillCombo(drltaikhoan, dt, "taikhoan", "taikhoan");
    }
    void load()
    {
        string strTK = drltaikhoan.Text.Trim();
        //string strTK = "131";
        string strMa_kh = txtMaKH.Text;
        crystalReport = new ReportDocument();
        crystalReport.Load(Server.MapPath("../ketoan/report_KeToan/KTCN_KhachHang_CT.rpt"));//file:///E:\PHAN MEM QUAN LY\quanlythuoc\report\toathuocnguoidung.rpt                      
        ...
        if (strTK != "")
        {
            sqltk = "select taikhoan,tentaikhoan from danhmuctk where taikhoan='" + strTK.Trim() + "'";
    ...
    private DataTable LoadCongNoKhachHang_CT(string strTK, string makh)
    {
        string sql;
        ...
        sql = "exec spCong_No_Khach_Hang_CT '" + ngay_dau_nam + "','" + StaticData.CheckDate_kt(txtNgay.Text) + "','" + StaticData.CheckDate_kt(txtDenNgay.Text) + "','" + strTK.Trim() + "','" + makh + "'";
    ...
    {
        if (drltaikhoan.Text.Equals("0") || txtMaKH.Text.Trim().Equals(""))
        {
            StaticData.MsgBox(this, "Bạn chưa chọn tài khoản hoặc nhà cung cấp!");
            ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "", "window.location.href('KTCN_Khach_Hang_CT.aspx.aspx')", true);
        }
        else
        load();
    }
