<%@ Page Language="C#" AutoEventWireup="true" CodeFile="KTTH_NhatKyThuTien.aspx.cs" Inherits="ketoan_KTTH_NhatKyThuTien" %>
<%@ Register Src="~/ketoan/Menu_KT/uscmenuKT_TongHop.ascx" TagName="uscmenuKT_TongHop" TagPrefix="uc1" %>
...
//???
function btnXem_Click()
{
    LoadNhatKyThuTien();
}

//????????
function LoadNhatKyThuTien()
{
    ...
    if(MaTaiKhoan.value!="")
    {
        xmlHttp = GetMSXmlHttp();
            ...
            //xmlHttp.open("GET","ajax.aspx?do=TestMaTaiKhoan&Key="+MaTaiKhoan.value+"&times="+Math.random(),true);
            //xmlHttp.send(null);
            xmlHttp.open("GET","ajax.aspx?do=LoadNhatKyThuTien&Key="+MaTaiKhoan.value+"&tungay="+txtNgay.value+"&denngay="+txtDenNgay.value+"&times="+Math.random(),true);
            xmlHttp.send(null);
    }
}
</script>
...
<table cellpadding = "0" cellspacing = "0" border = "0" width = "100%" >   
    <tr>
        <td width = "100%" align = "left" class="bg_menu">
            <uc1:uscmenuKT_TongHop ID="uscmenuKT_TongHop" runat="server" />
        </td>
    </tr>
    <tr>
			    ...
			    <table id="user" cellSpacing="1" cellPadding="1" width="100%" border="0" class = "khung">
				    <tr>
				        <td class="title" align = "center" style ="background-color: #4D67A2">
			                <span class="title" style ="color:#FFFFFF">NHẬT KÝ THU TIỀN </span></td>
				    </tr>
				    <TR>
